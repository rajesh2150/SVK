...
import { ADMIN_AUTH_STORAGE_KEY, getStoredOrders, type AdminOrder } from '../lib/sweetStore';
...
const AdminDashboardPage = () => {
  ...
  useEffect(() => {
    const handleNewOrder = () => {
      ...
      setNotifications(latestOrders.filter((order) => order.status === 'Pending'));
      playNotificationSound();
    };

    window.addEventListener('svk:new-order', handleNewOrder as EventListener);
    return () => window.removeEventListener('svk:new-order', handleNewOrder as EventListener);
  }, []);

  useEffect(() => {
    setNotifications(orders.filter((order) => order.status === 'Pending'));
  }, [orders]);
  ...
  const playNotificationSound = () => {
    try {
      ...
      audio.play().catch(() => {
        // Fallback to oscillator if play is blocked
